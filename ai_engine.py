...
class GameState:
    ...
    def get_payoff(self, opponent_board: Optional[Board] = None) -> Union[int, Dict[str, int]]:
        ...
        my_royalties = self.calculate_royalties(self.board) # Передаем self.board
        my_total_royalty = sum(my_royalties) # royalties is now jnp.ndarray, sum directly
        my_line_wins = 0
        ...
        opponent_royalties = self.calculate_royalties(opponent_board) # Передаем opponent_board
        opponent_total_royalty = sum(opponent_royalties) # royalties is now jnp.ndarray, sum directly

        #  Сравнение линий (scoring 1-6)
        my_bottom_rank, _ = self.evaluate_hand(jnp.array([card_to_array(card) for card in self.board.bottom]))
        opponent_bottom_rank, _ = self.evaluate_hand(jnp.array([card_to_array(card) for card in opponent_board.bottom]))
        if my_bottom_rank < opponent_bottom_rank:
            my_line_wins += 1
        elif my_bottom_rank > opponent_bottom_rank:
            my_line_wins -= 1

        my_middle_rank, _ = self.evaluate_hand(jnp.array([card_to_array(card) for card in self.board.middle]))
        opponent_middle_rank, _ = self.evaluate_hand(jnp.array([card_to_array(card) for card in opponent_board.middle]))
        if my_middle_rank < opponent_middle_rank:
            my_line_wins += 1
        elif my_middle_rank > opponent_middle_rank:
            my_line_wins -= 1

        my_top_rank, _ = self.evaluate_hand(jnp.array([card_to_array(card) for card in self.board.top]))
        opponent_top_rank, _ = self.evaluate_hand(jnp.array([card_to_array(card) for card in opponent_board.top]))
        if my_top_rank < opponent_top_rank:
            my_line_wins += 1
        elif my_top_rank > opponent_top_rank:
            my_line_wins -= 1
    ...
    def calculate_royalties_for_board(self, board: Board) -> Dict[str, int]: # This function is not used and redundant now
        """
        ...
        royalties_jax = temp_state.calculate_royalties(board) # Pass board here
        return {
            "top": int(royalties_jax[0]),
            "middle": int(royalties_jax[1]),
            "bottom": int(royalties_jax[2]),
        }
    ...
    def is_valid_fantasy_entry(self, board: Optional[Board] = None) -> bool:
        ...
        if top_rank == 8: # Пара
            top_cards_jax = jnp.array([card_to_array(card) for card in board.top]) # Get jax array for rank access
            rank_counts = jnp.bincount(top_cards_jax[:, 0], minlength=13)
            pair_rank_index = jnp.where(rank_counts == 2)[0][0]
            top_pair_rank = Card.RANKS[pair_rank_index]
            return top_pair_rank in ["Q", "K", "A"] # Проверяем что QQ или старше
        elif top_rank == 7: # Сет
            return True
        return False
...
def array_to_card(card_array: jnp.ndarray) -> Optional[Card]:
    ...
    return Card(Card.RANKS[int(card_array[0])], Card.SUITS[int(card_array[1])]) # Explicitly cast to int for indexing
...
def generate_placements(cards_jax: jnp.ndarray, board: Board, ai_settings: Dict, max_combinations: int = 10000) -> jnp.ndarray:
    ...
    is_dead_hand_vmap = jax.vmap(is_dead_hand_jax, in_axes=(0, None)) # Pass ai_settings as second argument
            ...
            if not can_repeat:
                ...
                if possible_actions:
                    ...
                    for action, temp_board in possible_actions:
                        temp_state = GameState(board=temp_board, ai_settings=game_state.ai_settings)
                        royalties = temp_state.calculate_royalties(temp_board)  #  TODO:  Переделать на JAX, Pass temp_board
                        total_royalty = sum(royalties) # royalties is now jnp.ndarray, sum directly
...
class CFRAgent:
    ...
    def get_move(self, game_state: GameState, timeout_event: Event, result: Dict) -> None:
        ...
        move_array = get_placement( # get_placement now returns jax array
        ...
        if move_array is None:  # Если get_placement вернул None (нет ходов)
            result["move"] = {"error": "Нет доступных ходов"}
            logger.debug("No actions available (get_placement returned None), returning error.")
            return

        move_dict = action_from_array(move_array) # Convert jax array back to dict for action
        result["move"] = move_dict
        logger.debug(f"Final selected move (from get_placement): {move_dict}")
        ...
        def play_one_batch(key):
            ...
            while not game_state_p0.board.is_full() or not game_state_p1.board.is_full():
                ...
                if not actions.shape[0] == 0:
                    ...
                    action = actions[action_index] # action is jax array now
                    ...
                    current_game_state = current_game_state.apply_action(action_from_array(action)) # Apply action from array, convert back to dict
...
def action_from_array(action_array: jnp.ndarray) -> Dict[str, List[Card]]:
    """Преобразует JAX-массив действий обратно в словарь действий."""
    action_dict = {"top": [], "middle": [], "bottom": [], "discarded": []}
    for i in range(3):
        card_array = action_array[i]
        if not jnp.array_equal(card_array, jnp.array([-1, -1])):
            action_dict["top"].append(array_to_card(card_array))
    for i in range(5):
        card_array = action_array[i + 3]
        if not jnp.array_equal(card_array, jnp.array([-1, -1])):
            action_dict["middle"].append(array_to_card(card_array))
    for i in range(5):
        card_array = action_array[i + 8]
        if not jnp.array_equal(card_array, jnp.array([-1, -1])):
            action_dict["bottom"].append(array_to_card(card_array))
    for i in range(2): # Up to 2 discarded cards are handled
        card_array = action_array[i + 13]
        if not jnp.array_equal(card_array, jnp.array([-1, -1])):
            action_dict["discarded"].append(array_to_card(card_array))
    return action_dict

def get_placement(selected_cards: List[Card], board: Board, discarded_cards: List[Card], ai_settings: Dict, baseline_evaluation) -> Optional[jnp.ndarray]: # Return jax array
    """
    Выбирает лучшее размещение карт, используя get_actions и baseline_evaluation.
    Возвращает JAX-массив, представляющий лучшее действие.
    """
    logger.debug("get_placement - START")
    game_state = GameState(selected_cards=selected_cards, board=board, discarded_cards=discarded_cards, ai_settings=ai_settings)
    possible_actions_jax = get_actions(game_state)

    if possible_actions_jax.shape[0] == 0:
        logger.debug("No valid actions found in get_actions, returning None")
        return None

    if ai_settings.get("agent_type") == "CFR":
        agent = ai_settings.get("agent_instance")
        if agent is None:
            logger.error("CFR Agent instance is not set in ai_settings for CFR agent_type.")
            return None
        info_set = game_state.get_information_set()
        info_hash = hash(info_set)

        if info_hash in agent.nodes_map and agent.nodes_mask[agent.nodes_map[info_hash]]: # Check if node is valid
            node_index = agent.nodes_map[info_hash]
            strategy = agent.get_average_strategy_by_index(node_index)
            if strategy.shape[0] != possible_actions_jax.shape[0]:
                logger.warning(f"Strategy size {strategy.shape[0]} != actions size {possible_actions_jax.shape[0]}. Using uniform strategy.")
                strategy = jnp.ones(possible_actions_jax.shape[0]) / possible_actions_jax.shape[0]
            else:
                strategy = strategy[:possible_actions_jax.shape[0]] # Ensure strategy size matches actions size

            key = random.PRNGKey(int(time.time_ns())) # Use nanoseconds for more entropy
            action_index = int(random.choice(key, jnp.arange(possible_actions_jax.shape[0]), p=strategy))
            best_action_jax = possible_actions_jax[action_index]
            logger.debug(f"CFR Agent selected action index: {action_index} from {possible_actions_jax.shape[0]} actions.")
            return best_action_jax # Return jax array

    elif ai_settings.get("agent_type") == "Random":
        key = random.PRNGKey(int(time.time_ns()))
        action_index = int(random.choice(key, jnp.arange(possible_actions_jax.shape[0])))
        best_action_jax = possible_actions_jax[action_index]
        logger.debug(f"Random Agent selected action index: {action_index} from {possible_actions_jax.shape[0]} actions.")
        return best_action_jax # Return jax array

    else: # Default to baseline evaluation (e.g., for Human agent or fallback)
        best_action_jax = None
        best_value = -float('inf')

        for action_jax in possible_actions_jax:
            action_dict = action_from_array(action_jax) # Convert jax array to dict for evaluation
            next_state = game_state.apply_action(action_dict)
            value = baseline_evaluation(next_state) # Assuming baseline_evaluation works with GameState
            if value > best_value:
                best_value = value
                best_action_jax = action_jax

        if best_action_jax is not None:
            logger.debug(f"Baseline Agent selected action with value: {best_value}")
            return best_action_jax # Return jax array
        else:
            logger.debug("No best action found by Baseline Agent, returning None")
            return None
